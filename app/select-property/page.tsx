...
import { ArrowLeft, User, Phone, Mail, Calendar, FileText, ShieldCheck } from "lucide-react"
...
export default function SelectPropertyPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    duration: "",
    additionalInfo: "",
    // New: buyer and parent Aadhar (metadata only)
    buyerAadhar: null as null | { name: string; size: number; type: string },
    parentAadhar: null as null | { name: string; size: number; type: string },
  })
  ...
  useEffect(() => {
    // One-time init to avoid infinite loops
    if (!isUserLoggedIn()) {
  ...
  }, []) // do not depend on searchParams to prevent re-runs
  ...
  // New: document upload for buyer and parent Aadhar (store metadata only)
  const handleDocUpload = (field: "buyerAadhar" | "parentAadhar") => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const selectionData = {
      ...
      docs: {
        buyerAadhar: formData.buyerAadhar,
    ...
    // Save selected property (existing behavior)
    localStorage.setItem("selectedProperty", JSON.stringify(property))

    // New: Save application record (metadata only)
    const existing = localStorage.getItem("rentalApplications")
    const applications = existing ? JSON.parse(existing) : []
    applications.push(selectionData)
    localStorage.setItem("rentalApplications", JSON.stringify(applications))
  ...
  if (isLoading || !isLoggedIn || !property) {
    return (
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-4 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            {/* NEW: Identity Documents (optional) */}
            <div className="rounded-lg border border-white/20 p-4 bg-white/5">
              <div className="flex items-center gap-2 mb-3">
                <ShieldCheck className="w-4 h-4 text-green-400" />
                <h4 className="text-white font-semibold text-sm sm:text-base">Identity Verification</h4>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-white mb-2 text-sm">Buyer Aadhar</label>
                  <input
                    type="file"
                    accept="image/*,application/pdf"
                    onChange={handleDocUpload("buyerAadhar")}
                  ...
                  />
                  {formData.buyerAadhar && (
                    <p className="text-gray-300 text-xs mt-2">
                      Uploaded: {formData.buyerAadhar.name} ({Math.round(formData.buyerAadhar.size / 1024)} KB)
                    </p>
                  )}
                </div>

                <div>
                  <label className="block text-white mb-2 text-sm">Parent Aadhar</label>
                  <input
                    type="file"
                    accept="image/*,application/pdf"
                    onChange={handleDocUpload("parentAadhar")}
                    className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                  />
                  {formData.parentAadhar && (
                    <p className="text-gray-300 text-xs mt-2">
                      Uploaded: {formData.parentAadhar.name} ({Math.round(formData.parentAadhar.size / 1024)} KB)
                    </p>
                  )}
                </div>
              </div>
